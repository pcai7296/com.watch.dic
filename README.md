<h1 align="center">腕上词典 — 336×480 矩形屏 适配版</h1>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/屏幕-336×480-ff6b35?style=flat-square" alt="Screens" />
...
本仓库是 **336×480（矩形屏）** 单分辨率适配分支，包名 `com.watch.dic.w336`。基于小米 `aiot-toolkit` 开发，内置 **20,000+ 条词汇**（数据源：ECDICT + CC-CEDICT + BNC/COCA 词族）。
...
- **📱 336×480 单屏适配** — 本分支针对 336×480 屏幕深度调优，布局、字号、间距全部按此分辨率定制
...
# 构建 336×480 RPK（npm run build:336）
npm run build:336
...
构建产物为 `dist/com.watch.dic.w336.debug.3.1.0.rpk`（已纳入 Git）。
...
| **屏幕** | 336×480 矩形屏 单屏适配，`designWidth: device-width` |
